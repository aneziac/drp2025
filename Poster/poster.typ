...
#let Davis_complex_CAT0 = [
  ...
  This result is impactful as it is a key step in showing the *contractability* of the Davis complex being *contractible* and it shows that the *word problem* (if two words represent the same element) and *conjugacy problem* (if two words represent conjugate elements) are solvable for $W$.
]
