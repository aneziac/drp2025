  ...
  #align(center + horizon)[
    #grid(
      ...
      canvas(length: 2cm, {
        ...
        geom_pt(s0, $t s$, ccolor0, "west")
        geom_pt(s1, $t$, ccolor0, "west")
        geom_pt(s2, $e$, ccolor0, "south-west")
        geom_pt(s3, $s$, ccolor0, "south-east")
        geom_pt(s4, $s t$, ccolor0, "south-east")
        geom_pt(s5, $s t s$, ccolor0, "east")
        geom_pt(s6, $s t s t = t s t s$, ccolor0, "north")
        geom_pt(s7, $t s t$, ccolor0, "west")
      })
    )
  ]

  Through the transformation $t := s r$, we can describe this group purely in terms of reflections, which gives rise to the notion of a *geometric reflection group*---a more generic group generated by reflections across a particular set of hyperplanes $cal(H)_i$ acting on Euclidean, hyperbolic, or spherical spaces.
...
#let acknowledgements = [
  We would like to thank our mentor Benedict Lee for his guidance and support in our readings. We also want to thank the Directed Reading Program at UCSB for the opportunity.
]
