...
#let tits_representation = [
  To better understand the abstract presentation of Coxeter groups, a key result by Jacques Tits gives a faithful representation
  $
    rho : W -> GL_n (RR),
  $
  with $n = |{s_i}|$, such that
  ...
  To construct this, we define a symmetric bilinear form $B$ by
  $
    B(e_i, e_j) = cases(
  ...
  $
  Then for a hyperplane $H_i = {v in RR^n : B (e_i, v) = 0}$ the linear map representatives $sigma_i : RR^n -> RR^n$ are defined by
  $
    sigma_i (v) = v - 2B(e_i, v) e_i
  $
  which is exactly what we might expect.
]
