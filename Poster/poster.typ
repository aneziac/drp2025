...
#let coxeter_systems = [
  *Dihedral groups*#h(-0.3em), well known from elementary group theory, encode the symmetries of regular polygons.
  ...
  Importantly, for some $T subset.eq S$, define the *parabolic subgroup* $W_T$ of $W$ by $W_T = gen(T)$; we can show that $(W_T, T)$ is a Coxeter System as well.
]
...
#let Davis_complex_CAT0 = [
  ...
  //$
  We say a *geodesic space* is *$"CAT"(0)$* if the triangles in the space appear no "fatter" than triangles in a Euclidean space of same dimension.
  ...
  // $
  be a chamber in $RR^n$ generated by the intersection of the half spaces produced by the hyperplanes $H_t$ for $t in T$ as in the *Tits Representation*#h(-0.3em).
  Then we can define the unique point $x_T$ in the interior of $C_T$ such that $d(x_T, H_t) = d_t$ for all $t in T$.
  We then metrize each cell of $Sigma(W, S)$, $w W_T$, as a copy of the polytope generated by the $W_T$-orbit of $x_T$ (a standard choice for $underline(d)$ is $d_s = 1/2$ for all $s$ constructing the path metric).
